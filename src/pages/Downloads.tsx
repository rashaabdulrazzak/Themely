...
import { deleteDownload, getDownloads } from "../services";
...
const Downloads: React.FC = () => {
  ...
  const [downloadToDelete, setDownloadToDelete] = useState<Download | null>(
      null
    );
    ...
    const handleDelete = async () => {
      ...
      try {
        ...
        toast.current?.show({
          severity: "success",
          summary: "Successful",
          detail: `Template "${downloadToDelete.fileName}" deleted successfully`,
          life: 3000,
        });
      } catch (error) {
  ...
  const confirmDeleteTemplate = (template: Download) => {
    setDownloadToDelete(template);
